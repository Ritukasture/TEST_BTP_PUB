CLASS zcl_mk_populate_rap_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_mk_populate_rap_data IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

   DATA: lt_prod_grs  TYPE TABLE OF ZTB_MK_PROD_GRP,
          lt_phases    TYPE TABLE OF ZTB_MK_PHASE,
          lt_markets   TYPE TABLE OF ZTB_MK_COUNTRY,
          lt_uom       TYPE TABLE OF ZTB_MK_UOM.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ZTB_MK_PROD_GRP.
*   insert the new table entries
    INSERT ZTB_MK_PROD_GRP FROM TABLE @lt_prod_grs.

*   check the result
    SELECT * FROM ZTB_MK_PROD_GRP INTO TABLE @lt_prod_grs.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ZTB_MK_PHASE.
*   insert the new table entries
    INSERT ZTB_MK_PHASE FROM TABLE @lt_phases.

*   check the result
    SELECT * FROM ZTB_MK_PHASE INTO TABLE @lt_phases.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ZTB_MK_COUNTRY.
*   insert the new table entries
    INSERT ZTB_MK_COUNTRY FROM TABLE @lt_markets.

*   check the result
    SELECT * FROM ZTB_MK_COUNTRY INTO TABLE @lt_markets.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ZTB_MK_UOM.
*   insert the new table entries
    INSERT ZTB_MK_UOM FROM TABLE @lt_uom.

*   check the result
    SELECT * FROM ZTB_MK_UOM INTO TABLE @lt_uom.
    out->write( sy-dbcnt ).
